...
/**
 * Core Widgets API
 *  Coppied from Wordpress wp-include
 *
...
function register_sidebars( $number = 1, $args = array() ) {
	...
	for ( $i = 1; $i <= $number; $i++ ) {
		...
		if ( $number > 1 )
			$_args['name'] = isset($args['name']) ? sprintf($args['name'], $i) : sprintf(modernsherbet__('Sidebar %d'), $i);
		else
			$_args['name'] = isset($args['name']) ? $args['name'] : modernsherbet__('Sidebar');
...
function register_sidebar($args = array()) {
	...
	$defaults = array(
		'name' => sprintf(modernsherbet__('Sidebar %d'), $i ),
		'id' => "sidebar-$i",
		'description' => '',
		'class' => '',
	...
	if ( $id_is_empty ) {
		...
		_doing_it_wrong( __FUNCTION__, sprintf( modernsherbet__( 'No %1$s was set in the arguments array for the "%2$s" sidebar. Defaulting to "%3$s". Manually set the %1$s to "%3$s" to silence this notice and keep existing sidebar content.' ), '<code>id</code>', $sidebar['name'], $sidebar['id'] ), '4.2.0' );
	}
...
function the_widget( $widget, $instance = array(), $args = array() ) {
	...
	if ( ! isset( $wp_widget_factory->widgets[ $widget ] ) ) {
		...
		_doing_it_wrong( __FUNCTION__, sprintf( modernsherbet__( 'Widgets need to be registered using %s, before they can be displayed.' ), '<code>register_widget()</code>' ), '4.9.0' );
		return;
	}
...
function wp_widget_rss_output( $rss, $args = array() ) {
	...
	if ( is_wp_error($rss) ) {
		if ( is_admin() || current_user_can('manage_options') )
			echo '<p><strong>' . modernsherbet__( 'RSS Error:' ) . '</strong> ' . $rss->get_error_message() . '</p>';
		return;
	}
	...
	if ( !$rss->get_item_quantity() ) {
		echo '<ul><li>' . modernsherbet__( 'An error has occurred, which probably means the feed is down. Try again later.' ) . '</li></ul>';
		$rss->__destruct();
		unset($rss);
		return;
	}
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		if ( empty( $title ) ) {
			$title = modernsherbet__( 'Untitled' );
		}
...
function wp_widget_rss_form( $args, $inputs = null ) {
	...
	if ( ! empty( $args['error'] ) ) {
		echo '<p class="widget-error"><strong>' . modernsherbet__( 'RSS Error:' ) . '</strong> ' . $args['error'] . '</p>';
	}

	$esc_number = esc_attr( $args['number'] );
	if ( $inputs['url'] ) :
?>
	<p><label for="rss-url-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'Enter the RSS feed URL here:' ); ?></label>
...
<?php endif; if ( $inputs['title'] ) : ?>
	<p><label for="rss-title-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'Give the feed a title (optional):' ); ?></label>
...
<?php endif; if ( $inputs['items'] ) : ?>
	<p><label for="rss-items-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'How many items would you like to display?' ); ?></label>
...
<?php endif; if ( $inputs['show_summary'] ) : ?>
	...
	<label for="rss-show-summary-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'Display item content?' ); ?></label></p>
<?php endif; if ( $inputs['show_author'] ) : ?>
	...
	<label for="rss-show-author-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'Display item author if available?' ); ?></label></p>
<?php endif; if ( $inputs['show_date'] ) : ?>
	...
	<label for="rss-show-date-<?php echo $esc_number; ?>"><?php modernsherbet_e( 'Display item date?' ); ?></label></p>
<?php
	endif;
